...
class Markov_Class:
  ...
  def generate_output(wtf, word_array, output_length):
    ...
    for word in word_array:
      if len(last_word) > 0 and last_word[-1] in sentence_delimiters:
        if word not in sentence_starters.keys():
          ...
          sentence_starters[word] += 1
      if len(last_word) > 0 and wtf.contains_character(clause_delimiters, last_word):
    ...
    # generate some text
    # TODO: weighting!
    generated_list = [random.choice(wtf.create_sample_pool(sentence_starters))]
    for i in range(output_length):
      last_word = generated_list[-1]
      next_word = ""
      if wtf.contains_character(sentence_delimiters, last_word) or last_word not in probs:
        next_word = random.choice(wtf.create_sample_pool(sentence_starters))
      if wtf.contains_character(clause_delimiters, last_word):
        next_word = random.choice(wtf.create_sample_pool(clause_starters))
      if last_word in probs:
        next_word = random.choice(wtf.create_sample_pool(probs[last_word]))
